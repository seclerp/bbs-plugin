 ...
 */
object BbsPaths {
    val bbsCmd: Path = Path("dotnet").resolve("bbs.cmd")
    val userSettingsFile: Path get() = bbsAppDataFolder.resolve("settings.user.json")
    val userProfilesDirectory: Path get() = bbsAppDataFolder.resolve("profiles")

    val bbsAppDataFolder: Path get() = userAppDataFolder.resolve("JetBrains").resolve("Bbs")
